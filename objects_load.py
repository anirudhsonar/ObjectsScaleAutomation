...
class UploadObjs(object):
  ...
  def _init_vars(self, endpoint, static_data):
    ...
    self._op_retries = self._kwargs.pop("op_retries")
  ...
  def _get_delete_prefix(self):
    if len(self._workload.keys()) == 1 and self._workload.keys()[0] == "delete":
      #print "Delete Prefix is empty, since only delete workload is running"
      return ""
  ...
  def nfsread(self, **kwargs):
    ...
    while not self._exit_test("NFS Read Workload"):
      if self._pause_workload("nfsread", 10):
        continue
      prefix=choice(self._total_ops_recorded["workload_to_prefix_map"]["nfsread"])
  ...
  def start_workload(self):
    ...
    while self._total_ops_recorded["vars"]["change_workload_detected"]:
      ...
      if not self._objectscluster:
        self._pc_object(pcip, pcuser, pcpass, objects_name, peip=peip,
                    peuser=peuser, pepass=pepass, **kwargs)
      try:
        self._start_and_monitor(self._workload.keys(), monitor_workloads,
                                **kwargs)
      except Exception as err:
        DEBUG("Exiting test. Error : %s, Logfile : %s"%(err, self.logfile))
        self._stop_workload("Hit road bloack")
  ...
  def enable_replication_relationship(self, bucket, enable_versioning,
      ...
      if create_new_bucket:
        self._create_bucket_on_destination(ip, self._access, self._secret,
  ...
  def _exit_test(self, workload, force=False):
  ...
  def _list_and_delete(self, **kwargs):
    ...
    self._validate_and_delete_objects(bucket, res,
            kwargs["skip_head_before_delete"], kwargs["skip_head_after_delete"],
            kwargs["skip_read_before_delete"])
    self._lock_unlock_bucket_for_delete(bucket, lock=False, unlock=True)
  ...
  def _list_and_read(self, bucket, objprefix, num_reads_from_list,
    ...
    if not istrunc:
      ...
      if all_versions and (len(all_versions) + len(prefixes) + \
        ...
        self._set_exit_marker(fatal, exception=''.join(format_exc()))
    if not all_versions:
      DEBUG("No Object found for read workload. Bucket : %s"%(bucket))
      sleep(1)
      return
  ...
  def _list_delimeter(self, bucket, delimeter="/", prefix="", marker="",
    ...
    with self._lock:
      ...
      if objs.get("IsTruncated"):
        ...
        WARN("Resetting makers for list workload. Bucket : %s, Istrucated : %s,"
             " Sleep for a sec"%(bucket, objs.get("IsTruncated")))
        self._bucket_info["list_bucket_map"][bucket]["vmarker"] = ""
        sleep(1)
  ...
  def _pc_object(self, pcip, pcuser, pcpass, objects_name,
                 peip, peuser, pepass, deplaoy=False, **kwargs):
  ...
  def _prepare_stage(self, **kwargs):
    ...
    if kwargs.get("skip_bucket_creation"):
      if not self._bucket_info["bucket_names"]:
        self._bucket_info["bucket_names"] = self._get_all_bucket_names(
                              kwargs.pop("bucket"), kwargs.pop("bucket_prefix"))
    ...
    if bucket:
      for bucketname in  bucket.split(","):
        self._create_bucket(bucketname, enable_versioning, **kwargs)
        self._bucket_info["bucket_names"].append(bucketname)
      self._update_read_map(kwargs["nfs_mount_options"])
      return
    ...
    for i in range(num_buckets):
      ...
      if not self._is_bucket_exists(bucket):
        self._create_bucket(bucket, enable_versioning, **kwargs)
        delay = 10
  ...
  def _set_exit_marker(self, fatal_msg, fatal=True, exception="",
    ...
    if self._time_to_exit or not fatal:
      ERROR("Caller : %s, %s, Ignoring since TimeToExit %s or Fatal is %s"
            %(caller, fatal_msg, self._time_to_exit, fatal))
      return
    with self._lock:
      if self._time_to_exit or not fatal:
        return
  ...
  def _stop_workload(self, reason=None, close_error_fh=True):
    ...
    DEBUG("Stop workload is called by %s"%(caller))
    ...
    INFO("Completing run. Reason : %s, Caller is : %s"%(reason, caller))
...
class Monitor():
  ...
  def check_live_workers(self):
    if self._counter%4 == 0:
      active_workers = self._testobj._task_manager.get_active_workers()
...
class ErrorInjection():
  def __init__(self, testobj, pcobject, vmops=None, components_for_ei=None):
    self._testobj = testobj
    self._pc = pcobject
  ...
  def is_compaction_running(self, podname):
...
class Utils():
  ...
  def _copy_file_to_remote(remoteip, localfile, remotefile, user, password):
    ssh = SSHClient()
    ssh.load_system_host_keys()
    ssh.set_missing_host_key_policy(AutoAddPolicy())
    ssh.set_missing_host_key_policy(MissingHostKeyPolicy())
...
class PCObjects():
  ...
  def _execute_rest_api_call(self, url, request_method,  data, timeout=120,
    ...
    while retry > 0:
      ...
      if res.status_code < 300:
        return res
      msg = "Error while executing %s (uuid %s). PC : %s (Objects name : %s), "\
            "Error : %s (Code:%s), URL : %s"%(request_method.__name__,
...
class NFSOps():
  ...
  def _nfs_debug_logging(self, msg):
  ...
  def _nfs_write(self, filename, data, file_open_flag, block_size, seek=0,
    ...
    self._nfs_debug_logging("%s position : %s (Expected : %s)"
    ...
    if read_md5 != datamd5 or len(data) != read_len:
      raise Exception("%s : Data Corruption. Expected md5 %s vs Found %s, "
                      "Expected size %s vs Found %s"
                      %(filename, datamd5, read_md5, len(data), read_len))
    self._nfs_debug_logging("%s, Md5 Expected %s vs Found %s, Size Expected %s "
        "vs Found %s"%(filename, datamd5, read_md5, len(data), read_len))
    return datamd5
    ...
    while True:
      ...
      if self._testobj._kwargs["sync_data_on_every_write_to_file"]:
        self._execute(self._posix.nfs_op_flush_file, fh=write_fh,
                       retry_count=0, set_exit_marker_on_failure=False)
      self._nfs_debug_logging("Writing completed. file %s, @ offset start : %s,"
  ...
  def _close_fh(self, filename, fd, ignore_failure=False, retry_count=0,
                set_exit_marker_on_failure=False):
    if not fd:
      self._nfs_debug_logging("Filename : %s, Filehandle is NONE"%filename)
      return
    try:
      if self._testobj._kwargs["sync_data_on_file_close"]:
        self._nfs_debug_logging("Filename : %s, Fsyncing data"%filename)
        self._execute(self._posix.nfs_op_fsync, fh=fd,
                     retry_count=retry_count,
                     set_exit_marker_on_failure=set_exit_marker_on_failure)
        self._nfs_debug_logging("Filename : %s, Flushing file"%filename)
        self._execute(self._posix.nfs_op_flush_file, fh=fd,
                     retry_count=retry_count,
                     set_exit_marker_on_failure=set_exit_marker_on_failure)
        self._nfs_debug_logging("Filename : %s, Fsynced/Flushed data"%filename)
      if isinstance(fd, file):
        self._nfs_debug_logging("Filename : %s, closing file"%filename)
        self._execute(fd.close, retry_count=retry_count,
                       set_exit_marker_on_failure=set_exit_marker_on_failure)
        self._nfs_debug_logging("Filename : %s, closed file"%filename)
      else:
  ...
  def _open_fh(self, filename, mode, retry_count=0, use_os_mod=False, seek=None,
    ...
    if use_os_mod:
      ...
      fh = self._execute(self._posix.nfs_op_open_fh, filename=filename,
                   ...
                   **kwargs)
      if seek:
        self._nfs_debug_logging("Filename : %s, seeking file to : %s"
                                %(filename, seek))
    ...
    self._nfs_debug_logging("Filename : %s is opened and seeked : %s."
                             %(filename, seek))
    return fh
...
class Stats():
  ...
  def _init_tables(self):
    ...
    self.workload_stats = PrettyTable([y+"IP", "Count", "Workload",
          "Object Size", "Start Time", "Endtime", "RunTime", "Memory Usage",
          "WL Iteration"+e])
    ...
    for item in self._total_ops_recorded["workload_profiles"]:
      ...
      self.workload_stats.add_row(
                         ...
                         [self._utils.convert_size(i) for i in item["objsize"]],
                          item["start_time"], item["end_time"], runtime,
  ...
  def list_objects(self):
    ...
    self.stats.add_row([g+"S3LIST"+e,
                        "(Objs:%s / Prefix:%s)"%(list_objs, num_pref),
                        "N/A", "N/A", "N/A"])
  ...
  def delete_objects(self):
    ...
    self.stats.add_row([g+"S3DELETE"+e, total_deletes, size,
  ...
  def common(self):
    ...
    if t_failures > 0:
      total_failures_recorded = "%s(U:%s)"%(t_failures,
                 (t_failures-(i_failures + e_failures)))
      self.stats.add_row([f+"Total.Failed.OPs"+e,total_failures_recorded,"N/A",
                     f+"Last.Failure.Time"+e,
                     self._total_ops_recorded.get("Last.Recorded.Time")])
    if  i_failures> 0:
      self.stats.add_row([w+"Ignored.Failures"+e, i_failures,"N/A",
                         "N/A", "N/A"])
    if e_failures > 0:
      self.stats.add_row([b+"Expected.Failures"+e, e_failures,"N/A",
                         "N/A", "N/A"])
    #self._add_summary()
...
class OpExecutor():
  def __init__(self, uploadobj,  error_fh_tracker, **kwargs):
    ...
    if self._objects._op_retries:
      for ops in self._objects._op_retries.split(","):
        op, retry = ops.split(":")
        self._op_retries[op.strip()] = int(retry.strip())
  ...
  def _execute(self, method, **kwargs):
    retry_count = kwargs.pop("retry_count", self._op_retries.get(
                                  method.__name__, self._objects._retry_count))
    retry_delay = kwargs.pop("retry_delay", self._objects._retry_delay)
...
class RemoteExecution():
  ...
  def log_all_stats(self, interval, debug_stat_interval, ignore_errors):
    test_time = self._get_test_start_time(timeout=1800)
  ...
  def _process_stats(self, log_debug_stats, ignore_errors, timeout=300):
    ...
    INFO("Processing logs (Time Took : %s): \nAlive clients : %s (Total : %s)\n"
         "Dead Clients : %s (Total : %s)\nClients not running test: "
          ...
          res["corrupt_clients"]))
    print "*"*200
    all_stats = self._merge_all_client_stats(res["active_clients"])
  ...
  def _reboot_client_and_restart_test(self, client):
    ...
    if self._db['clients'][client]["num_retries_to_start_test"
      ...
      print "We tried restarting test in %s for more than 20 times, giving up "\
            "now"%client
      return
  ...
  def _get_records_from_remote_client(self, client, filename, logfname):
    ...
    try:
      ...
      print "Trying to use last fetched stats from DB for client : %s"%client
  ...
  def _copy_clients_records_logs(self, client, records, filename, logfname):
    ...
    try:
      ...
      print "ERROR : Failed to copy file from %s. File : %s. Err : %s"%(
  ...
  def _merge_all_client_stats(self, alive_clients):
    final_records = {}
    for client in alive_clients:
      stats = self._db['clients'][client]["active_stats"]
  ...
  def _get_client_stat(self, client, ignore_errors, get_stats_only=False):
    ...
    try:
      ...
      print "Not able to fetch stats from client : %s. Err : %s"%(client, err)
      if not ignore_errors:
        raise
    if get_stats_only:
      return res
    if not res:
      if self._db['clients'][client]["status"] in [Constants.TEST_NOT_STARTED,
        ...
        print "%s : Test doesnt seems to have started"%client
        return
      if self._db['clients'][client]["status"] != Constants.TEST_NOT_RUNNING:
  ...
  def _check_workload_on_remote_client(self, client, workloads_to_check=None):
    if workloads_to_check is None:
      workloads_to_check = ["nfsrename", "nfscopy", "nfsvdbench",
                         "nfsoverwrite", "nfsfio", "nfswrite"]
    try:
  ...
  def _stop_test(self, wait_for_cleanup=True, timeout=2400, ignore_errors=True):
    ...
    while (etime - time()) > 0 :
      ...
      INFO("Waiting for all clients to stop test. Time elapsed : %s secs"
           %((time()+timeout)-etime))
      sleep(30)
    raise Exception("Timeout after %s, Failed to stop all clients."%timeout)
  ...
  def _remote_test_execution(self, remote_script_location,
    ...
    for client in self._clients:
      ...
      self._db['clients'][client].update({"status" : Constants.TEST_STARTING,
                                          "start_attempt_time":datetime.now()})
      self._db['clients'][client]["num_retries_to_start_test"] += 1
    print "Will wait for all workers to start test on clients"
    self._task_manager.wait_for_all_worker_to_be_free()

  def reboot_all_clients(self, remote_script_location, vmprefix):
    self._db["test_args"]["remote_script_location"] = remote_script_location
    if self._aoscluster:
      INFO("Resetting all clients")
    ...
    sleep(60)
    self.is_workload_running(remote_script_location)

  def is_workload_running(self, remote_script_location):
    for client in self._clients:
      self._task_manager.add(self._is_wl_running, client=client,
                  script_location=remote_script_location,
                  quota_type=Constants.REMOTE_POPS_QUOTA)
    print "Will wait for all workers get workload status from clients"
    self._task_manager.wait_for_all_worker_to_be_free()

  def _is_wl_running(self, client, script_location):
  ...
  def mount_all_buckets(self, nfs_mount_for_logs):
    ...
    print "Will wait for all workers to mount logs share on all clients"
    self._task_manager.wait_for_all_worker_to_be_free()
  ...
  def deploy_remote_clients(self, peip, username, password, vmname_prefix,
                            vm_image_name,  vm_image_url, storage_ctr,
                            network_name, num_clones, memory_in_mb, num_vcpus,
                            num_cores_per_vcpu, network_vlan):
    if self._clients:
      self._init_db()
      return self._clients
    INFO("Deploying %s clients for load test"%num_clones)
    if not self._aoscluster:
      self._aoscluster = AOSCluster(peip, username, password)
    clients = self._aoscluster.create_vm_from_image(vmname_prefix,
                vm_image_name, vm_image_url, storage_ctr, network_name,
                num_clones, memory_in_mb, num_vcpus, num_cores_per_vcpu,
                network_vlan)
  ...
  def start_test_on_remote_client(self, local_script_location,
                        cfgfile_for_remote_execution, remote_test_location,
                        remote_cmdline_args, restart_test_if_required,
                        use_unique_bucket_prefix, ntp_server,
                        reboot_all_clients_before_test, use_nfs_mount_for_logs,
                        nfs_mount_for_logs, remote_vm_prefix):
    ...
    if reboot_all_clients_before_test:
      self.reboot_all_clients(remote_script_location, remote_vm_prefix)
      if use_nfs_mount_for_logs:
        self.mount_all_buckets(nfs_mount_for_logs)
    if self._is_test_running():
      if not restart_test_if_required:
    ...
    self._remote_test_execution(remote_script_location, remote_cfgfile_location,
        remote_cmdline_args, use_unique_bucket_prefix=use_unique_bucket_prefix,
  ...
  def run(self):
    self._exit_on_cntr_c()
    ignore_errors = True
    self.deploy_remote_clients(peip=self._kwargs["remote_clients_peip"],
         username=self._kwargs["remote_clients_pe_admin_user"],
         password=self._kwargs["remote_clients_pe_admin_password"],
         vmname_prefix=self._kwargs["remote_clients_vmname"],
         vm_image_name=self._kwargs["remote_clients_vmimage"],
         vm_image_url=self._kwargs["remote_clients_vmimage_url"],
         storage_ctr=self._kwargs["remote_clients_storage_ctr"],
         network_name=self._kwargs["remote_clients_network_name"],
         num_clones=self._kwargs["num_remote_clients"],
         memory_in_mb=self._kwargs["remote_clients_memory"],
         num_vcpus=self._kwargs["remote_clients_vcpu"],
         num_cores_per_vcpu=self._kwargs["remote_clients_cores_per_vcpu"],
         network_vlan=self._kwargs["remote_clients_network_vlanid"])
    if self._kwargs["cfgfile_for_remote_execution"] and not self._kwargs[
                                              'remote_execution_monitor_only']:
      self.start_test_on_remote_client(self._kwargs["local_script_location"],
        self._kwargs["cfgfile_for_remote_execution"],
        self._kwargs["remote_test_location"],
        self._kwargs["remote_cmdline_args"],
        self._kwargs["restart_test_if_required"],
        use_unique_bucket_prefix=self._kwargs["use_unique_bucket_prefix"],
        ntp_server=self._kwargs["ntp_server"],
        reboot_all_clients_before_test=self._kwargs["reboot_all_clients_before_test"],
        use_nfs_mount_for_logs=self._kwargs["use_nfs_mount_for_logs"],
        nfs_mount_for_logs=self._kwargs["nfs_mount_for_logs"],
        remote_vm_prefix=self._kwargs["remote_clients_vmname"])
...
class WorkloadRunner():
  ...
  def arg_parser(self):
    ...
    parser.add_argument("--op_retries", required=False, type=str, help="Delay "
                        "between each retry for specific op. Default:None."
                        "Expected format : op_name:num_retries", default=None)
    ...
    #prefix, expiry, version_expiry, abort_multipart
  ...
  def _prepare_log_paths(self, log_path, symlinkfilename):
    ...
    try:
      ...
      print "Failed to create symlink from %s to %s. Err : %s"%(logpath,
                                                               dir_symlink, err)
